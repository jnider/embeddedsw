...
*
*******************************************************************************/
static AieRC XAie_SysfsIO_Finish(void *IOInst)
{
	XAie_SysfsIO *DevInst = (XAie_SysfsIO *)IOInst;
	free(DevInst->SysfsAddress);
	free(DevInst->SysfsValue);
	free(DevInst);
...
*
*******************************************************************************/
static AieRC XAie_SysfsIO_Init(XAie_DevInst *DevInst)
{
	XAie_SysfsIO *IOInst;

	IOInst = (XAie_SysfsIO *)malloc(sizeof(*IOInst));
	if(IOInst == NULL) {
	...
	IOInst->SysfsAddress= malloc(strlen((const char *)DevInst->IOInst) + strlen("/address") + 1);
	sprintf(IOInst->SysfsAddress, "%s/address", (const char *)DevInst->IOInst);

	IOInst->SysfsValue = malloc(strlen((const char *)DevInst->IOInst) + strlen("/value") + 1);
	sprintf(IOInst->SysfsValue, "%s/value", (const char *)DevInst->IOInst);
...
*
*******************************************************************************/
static AieRC XAie_SysfsIO_Write32(void *IOInst, u64 RegOff, u32 Value)
{
	char buf[20];
	XAie_SysfsIO *SysfsIOInst = (XAie_SysfsIO *)IOInst;

	XAIE_DBG("W: 0x%lx, 0x%x\n", RegOff, Value);

	int address = open(SysfsIOInst->SysfsAddress, O_WRONLY);
	if (address == -1) {
		XAIE_ERROR("Error opening %s\n", SysfsIOInst->SysfsAddress);
		return XAIE_ERR;
	}
	...
	int value = open(SysfsIOInst->SysfsValue, O_WRONLY);
	if (value == -1) {
		XAIE_ERROR("Error opening %s\n", SysfsIOInst->SysfsValue);
		return XAIE_ERR;
	}
...
*
*******************************************************************************/
static AieRC XAie_SysfsIO_Read32(void *IOInst, u64 RegOff, u32 *Data)
{
	char buf[20];
	XAie_SysfsIO *SysfsIOInst = (XAie_SysfsIO *)IOInst;
	...
	int value = open(SysfsIOInst->SysfsValue, O_RDONLY);
	if (value == -1) {
		XAIE_ERROR("Error opening %s\n", SysfsIOInst->SysfsValue);
		return XAIE_ERR;
	}
...
*
*******************************************************************************/
static AieRC XAie_SysfsIO_MaskWrite32(void *IOInst, u64 RegOff, u32 Mask,
		u32 Value)
{
	u32 RegVal;

	AieRC rc = XAie_SysfsIO_Read32(IOInst, RegOff, &RegVal);
	if (rc != XAIE_OK)
		return rc;

	RegVal &= ~Mask;
	return XAie_SysfsIO_Write32(IOInst, RegOff, (RegVal | Value));
}
...
*
*******************************************************************************/
static AieRC XAie_SysfsIO_MaskPoll(void *IOInst, u64 RegOff, u32 Mask, u32 Value,
		u32 TimeOutUs)
{
	u32 Count, RegVal;
	...
	do {
		XAie_SysfsIO_Read32(IOInst, RegOff, &RegVal);
...
*
*******************************************************************************/
static AieRC XAie_SysfsIO_BlockWrite32(void *IOInst, u64 RegOff, u32 *Data,
		u32 Size)
{
	for(u32 i = 0U; i < Size; i ++) {
		XAie_SysfsIO_Write32(IOInst, RegOff + i * 4U, *Data);
		Data++;
	}
...
*
*******************************************************************************/
static AieRC XAie_SysfsIO_BlockSet32(void *IOInst, u64 RegOff, u32 Data,
		u32 Size)
{
	for(u32 i = 0U; i < Size; i++)
		XAie_SysfsIO_Write32(IOInst, RegOff+ i * 4U, Data);
...
static AieRC XAie_SysfsIO_CmdWrite(void *IOInst, u8 Col, u8 Row, u8 Command,
...
*
*******************************************************************************/
static void _XAie_SysfsIO_NpiWrite32(void *IOInst, u32 RegOff,
		u32 RegVal)
{
	XAie_SysfsIO *SysfsIOInst = (XAie_SysfsIO *)IOInst;
	u64 RegAddr;
	...
	RegAddr = SysfsIOInst->NpiBaseAddr + RegOff;
}
...
*
*******************************************************************************/
static AieRC _XAie_SysfsIO_NpiMaskPoll(void *IOInst, u64 RegOff, u32 Mask,
		u32 Value, u32 TimeOutUs)
{
	XAie_SysfsIO *SysfsIOInst = (XAie_SysfsIO *)IOInst;

	XAIE_DBG("NPIMP: 0x%lx, 0x%x, 0x%x, 0x%d\n", SysfsIOInst->NpiBaseAddr + RegOff,
...
*
*******************************************************************************/
static AieRC XAie_SysfsIO_RunOp(void *IOInst, XAie_DevInst *DevInst,
		     XAie_BackendOpCode Op, void *Arg)
{
	...
	switch(Op) {
		case XAIE_BACKEND_OP_NPIWR32:
		{
			...
			_XAie_SysfsIO_NpiWrite32(IOInst, Req->NpiRegOff,
					Req->Val);
			break;
		}
		case XAIE_BACKEND_OP_NPIMASKPOLL32:
		{
			...
			return _XAie_SysfsIO_NpiMaskPoll(IOInst, Req->NpiRegOff,
...
*
*******************************************************************************/
static XAie_MemInst* XAie_SysfsMemAllocate(XAie_DevInst *DevInst, u64 Size,
...
*
*******************************************************************************/
static AieRC XAie_SysfsMemFree(XAie_MemInst *MemInst)
{
	XAIE_DBG("%s\n", __func__);
	free(MemInst->VAddr);
	free(MemInst);
...
*
*******************************************************************************/
static AieRC XAie_SysfsMemSyncForCPU(XAie_MemInst *MemInst)
{
	(void)MemInst;
...
*
*******************************************************************************/
static AieRC XAie_SysfsMemSyncForDev(XAie_MemInst *MemInst)
{
	(void)MemInst;
...
static AieRC XAie_SysfsMemAttach(XAie_MemInst *MemInst, u64 MemHandle)
{
	(void)MemInst;
	(void)MemHandle;
...
static AieRC XAie_SysfsMemDetach(XAie_MemInst *MemInst)
{
	(void)MemInst;
...
static u64 XAie_SysfsGetTid(void)
{
#ifdef __linux__
...
const XAie_Backend SysfsBackend =
{
	.Type = XAIE_IO_BACKEND_SYSFS,
	.Ops.Init = XAie_SysfsIO_Init,
	.Ops.Finish = XAie_SysfsIO_Finish,
	.Ops.Write32 = XAie_SysfsIO_Write32,
	.Ops.Read32 = XAie_SysfsIO_Read32,
	.Ops.MaskWrite32 = XAie_SysfsIO_MaskWrite32,
	.Ops.MaskPoll = XAie_SysfsIO_MaskPoll,
	.Ops.BlockWrite32 = XAie_SysfsIO_BlockWrite32,
	.Ops.BlockSet32 = XAie_SysfsIO_BlockSet32,
	.Ops.CmdWrite = XAie_SysfsIO_CmdWrite,
	.Ops.RunOp = XAie_SysfsIO_RunOp,
	.Ops.MemAllocate = XAie_SysfsMemAllocate,
	.Ops.MemFree = XAie_SysfsMemFree,
	.Ops.MemSyncForCPU = XAie_SysfsMemSyncForCPU,
	.Ops.MemSyncForDev = XAie_SysfsMemSyncForDev,
	.Ops.MemAttach = XAie_SysfsMemAttach,
	.Ops.MemDetach = XAie_SysfsMemDetach,
	.Ops.GetTid = XAie_SysfsGetTid,
	.Ops.SubmitTxn = NULL,
};
